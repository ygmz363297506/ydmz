/*******************************************************************************
 * Author: Erwin Yusrizal UX by: Tai Nguyen Contributors: James West
 * <jwwest@gmail.com> JP Adams <jpadamsonline@gmail.com> James Warwood
 * <james.duncan.1991@googlemail.com> Craig Roberts <craig0990@googlemail.com>
 * Created On: 27/02/2013 Version: 2.7.2 Features & Bugs:
...
(function($, window, document, undefined) {
	 ...
	 * GLOBAL VAR
	 */
	var _globalWalkthrough = {}, _elements = [], _activeWalkthrough, _activeId, _hasDefault = true, _counter = 0, _isCookieLoad, _firstTimeLoad = true, _onLoad = true, _index = 0, _isWalkthroughActive = false, $jpwOverlay = $('<div id="jpwOverlay"></div>'), $jpWalkthrough = $('<div id="jpWalkthrough"></div>'), $jpwTooltip = $('<div id="jpwTooltip"></div>');
	...
	var methods = {
		...
		init : function(options) {
			...
			// @todo what happens with multiple walkthroughs on the same
			// element?
			...
			return this.each(function(i) {
				...
				// set the activeWalkthrough if onLoad is false for all
				// walkthroughs
	...
	function showStep(skipDirection) {
		var options = _activeWalkthrough, step = options.steps[_index], targetElement = options._element.find(step.wrapper), scrollTarget = getScrollParent(targetElement), maxScroll, scrollTo;
		...
		// For modals, scroll to the . For tooltips, try and center the target
		...
		if (scrollTarget.scrollTop() !== scrollTo
				&& ((scrollTarget.scrollTop() === maxScroll && scrollTo < maxScroll) || (scrollTo <= 0 && scrollTarget.scrollTop() > 0) || (scrollTo > 0))) {
	...
	function buildWalkthrough() {
		...
		var options = _activeWalkthrough, step = options.steps[_index], targetElement, scrollParent, maxHeight;
		...
		if (step.popup.type !== 'modal' && step.popup.type !== 'nohighlight') {
			...
			if (step.wrapper === '' || typeof step.wrapper === 'undefined') {
				...
				debug('Your walkthrough position is: "' + step.popup.type + '" but wrapper is empty or undefined. Please check your "' + _activeId
						+ '" wrapper parameter.');
				return;
			}
			...
			// If max height is negative, means we have plenty room so
			// targetElement
			// height
			maxHeight = maxHeight <= 0 ? targetElement.outerHeight() : maxHeight;

			// @todo make it so we don't have to destroy and recreate this
			// element for
			// each step
			...
			$('<div>').addClass('overlay-hole').height(Math.min(maxHeight, targetElement.outerHeight() + 30)).width(targetElement.outerWidth() + 30).css({
				...
				padding : '20px',
				position : 'absolute',
				top : targetElement.offset().top - 15,
	...
	function showModal() {
		var options = _activeWalkthrough, step = options.steps[_index];
		...
		var tooltipSlide = $('<div id="tooltipTop">' + '<div id="topLeft"></div>' + '<div id="topRight"></div>' + '</div>' +

		'<div id="tooltipInner">' + '</div>' +

		'<div id="tooltipBottom">' + '<div id="bottomLeft"></div>' + '<div id="bottomRight"></div>' + '</div>');
	...
	function showTooltip() {
		var opt = _activeWalkthrough, step = opt.steps[_index];

		var top, left, arrowTop, arrowLeft, overlayHoleWidth = $('#jpWalkthrough .overlay-hole').outerWidth(), overlayHoleHeight = $(
				'#jpWalkthrough .overlay-hole').outerHeight(), overlayHoleTop = $('#jpWalkthrough .overlay-hole').offset().top, overlayHoleLeft = $(
				'#jpWalkthrough .overlay-hole').offset().left, arrow = 30;

		var textRotation = (typeof step.popup.contentRotation === 'undefined' || parseInt(step.popup.contentRotation, 10) === 0) ? clearRotation()
		...
		case 'bottom':
			...
			left = (overlayHoleLeft + (overlayHoleWidth / 2)) - ($jpwTooltip.width() / 2) - 5 + parseInt(step.popup.offsetHorizontal, 10) - 100;
			arrowLeft = (($jpwTooltip.width() / 2) - arrow) + parseInt(step.popup.offsetArrowHorizontal, 10);
			arrowTop = (step.popup.offsetArrowVertical) ? parseInt(step.popup.offsetArrowVertical, 10) : '';
			break;
		case 'left':
	...
	/*
	 * Get the content for a step. First attempts to treat step.popup.content as
	 * a selector. If this fails, or returns an empty result set, it falls back
	 * to return the value of step.popup.content.
	 * 
	 ...
	 * 
	 * @param {String} id The button identifier within the options.buttons hash
	 * (e.g. 'jpwNext') @param {jQuery|String} appendTo (Optional) The element
	 * or selector to append the button to. Defaults to #tooltipWrapper
	 */
	function showButton(id, appendTo) {
		...
		var btn = _activeWalkthrough.buttons[id], $a;
	...
	/*
	 * Get the first scrollable parent of the specified element. Adapted from
	 * jQueryUI's [scrollParent](api.jqueryui.com/scrollParent/).
	 * 
	 * @param {jQuery} element The element to find the scrollable parent of
	 * 
	 * @return {jQuery} The first scrollable parent, or an empty jQuery object
	 * if either of the following is true: 1. `element`'s position is 'fixed' 2.
	 * `element`'s position is 'absolute', and the parent's is 'static'
	 */
	function getScrollParent(element) {
		...
		var position = element.css('position'), excludeStaticParent = position === 'absolute', scrollParent = element.parents().filter(function() {
			var parent = $(this);
	...
	$.pagewalkthrough = $.fn.pagewalkthrough = function(method) {
		...
		if (methods[method]) {
			...
			// render the overlay on it has a default walkthrough set to show
			// onload
	...
	$.fn.pagewalkthrough.defaults = {
		...
		steps : [ {
			...
			popup : {
				// Selector for the element which contains the content, or the
				// literal
				// content
				content : '',
				...
				type : 'modal',
				// Position for tooltip and nohighlight style popups - either
				// top,
				...
				contentRotation : 0,
				// If set to 'skip', skips tooltip/nohighlight types when the
				// wrapper
				// does not exist. If set to anything else, uses the value as a
				// fallback
				// popup type (e.g. 'modal' will fallback to a popup type of
				// 'modal').
				fallback : 'skip'
			},
		...
		// **(Required)** Walkthrough name. Should be a unique name to identify
		// the
		// walkthrough, as it will
		// be used in the cookie name
		name : null,
		// Automatically show the walkthrough when the page is loaded. If
		// multiple
		...
		onRestart : $.noop,
		// Callback executed when the walkthrough is closed. The walkthrough can
		// be
		// closed by the user clicking the close button in the top right, or
		// clicking the finish button on the last step
		onClose : $.noop,
		// Callback executed when cookie has been set after a walkthrough has
		// been
		// closed
		onCookieLoad : $.noop,
		/*
		 ...
		 * 
		 * Hash of buttons to show. Object keys are used as the button element's
		 * ID
		 */
		buttons : {
			// ID of the button
			jpwClose : {
				...
				i18n : '关闭本节向导',
				// Whether or not to show the button. Can be a boolean value, or
				// a
